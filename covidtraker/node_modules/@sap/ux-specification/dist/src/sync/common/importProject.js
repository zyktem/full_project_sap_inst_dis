...
function getAnnotationsForUi(oDataServiceAVT) {
    ...
    for (const entity of allEntityTypes) {
        ...
        if (LineItem &&
            !LineItem.qualifier &&
            LineItem.find((li) => li.$Type === "com.sap.vocabularies.UI.v1.DataField" /* UIAnnotationTypes.DataField */ || li.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAnnotation" /* UIAnnotationTypes.DataFieldForAnnotation */)) {
...
function generateObjectPageSchemasV2(generateParameters, genericSchemas, appSchemas) {
    const allPages = (0, appProvider_1.getPages)(generateParameters.manifest[ux_specification_types_1.ManifestSection.generic].pages, ux_specification_types_1.v2.FE_TEMPLATE_V2_OBJECT_PAGE);
...
function generateObjectPageSchemasV4(generateParameters, genericSchemas, appSchemas) {
    ...
    for (const page in pages) {
        ...
        const genericSchemaFile = (0, utils_2.getSchemaFilePath)(ux_specification_types_1.SchemaType.ObjectPage);
        appSchemas[schemaFile.filename] = (0, generate_2.generateObjectPageSchemaV4)(generateParameters, genericSchemas[genericSchemaFile.filename], contextPath);
    }
}
/**
...
function generateAppSchemasV2(generateParameters, genericSchemas, uiEntityTypes, templateType) {
    ...
    if (templateType === ux_specification_types_1.TemplateType.ListReportObjectPageV2 && uiEntityTypes) {
        /*--- List Report ----*/
        const allPages = (0, appProvider_1.getPages)(generateParameters.manifest[ux_specification_types_1.ManifestSection.generic].pages, ux_specification_types_1.v2.FE_TEMPLATE_V2_LIST_REPORT);
        for (const page in allPages) {
            const entitySetName = allPages[page].entitySet;
        ...
        const allPages = (0, appProvider_1.getPages)(generateParameters.manifest[ux_specification_types_1.ManifestSection.generic].pages, ux_specification_types_1.v2.FE_TEMPLATE_V2_ALP);
        for (const page in allPages) {
            const entitySetName = allPages[page].entitySet;
            generateParameters.entitySet = getEntitySetByEntitySetName(entitySetName, generateParameters.serviceAVT);
...
function generateCustomPageSchemasV4(generateParameters, genericSchemas, appSchemas) {
    ...
    for (const pageId in pages) {
        ...
        const viewData = (0, utils_1.getPageCustomViewFile)(generateParameters.views || [], pageId, generateParameters.manifest);
        ...
        if (viewData) {
            ...
            appSchemas[schemaFile.filename] = (0, generate_2.generateCustomPageSchemaV4)(genericSchemas[genericSchemaFile.filename], viewData, generateParameters.serviceAVT, generateParameters.logger);
        }
    }
}
/**
...
async function importProjectSchema(importProjectParameters) {
    // Initialize i18next
    (0, i18n_1.initI18n)();
    const fioriElementsVersion = (0, utils_2.getVersionFromManifest)(importProjectParameters.manifest);
    const oDataServiceAVT = (0, utils_2.parseAndMergeAndConvert)(importProjectParameters.annotations, importProjectParameters.logger);
    const uiEntityTypes = getAnnotationsForUi(oDataServiceAVT);
    const templateType = (0, utils_2.getTemplateTypeFromManifest)(importProjectParameters.manifest, fioriElementsVersion, importProjectParameters.logger);
