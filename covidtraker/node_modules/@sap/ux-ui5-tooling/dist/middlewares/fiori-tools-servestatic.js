...
var require_aproba = __commonJS({
  "../../node_modules/@ui5/logger/node_modules/aproba/index.js"(exports2, module2) {
    "use strict";
    ...
    var validate = module2.exports = function(rawSchemas, args2) {
      ...
      if (!types.A.check(args2))
        throw invalidType(1, ["array"], args2);
      ...
      var matching = arity[args2.length];
      if (!matching) {
        throw wrongNumberOfArgs(Object.keys(arity), args2.length);
      }
      for (var ii = 0; ii < args2.length; ++ii) {
        var newMatching = matching.filter(function(schema) {
          var type = schema[ii];
          var typeCheck = types[type].check;
          return typeCheck(args2[ii]);
        });
        if (!newMatching.length) {
          ...
          throw invalidType(ii, labels, args2[ii]);
        }
    ...
    function wrongNumberOfArgs(expected, got) {
      ...
      var args2 = expected.every(function(ex) {
        return ex.length === 1;
      }) ? "argument" : "arguments";
      return newException("EWRONGARGCOUNT", "Expected " + english + " " + args2 + " but got " + got);
    }
...
var require_log = __commonJS({
  "../../node_modules/@ui5/logger/node_modules/npmlog/log.js"(exports2, module2) {
    ...
    var EE = require("events").EventEmitter;
    var log3 = exports2 = module2.exports = new EE();
    var util = require("util");
    ...
    log3.log = function(lvl, prefix, message2) {
      ...
      if (stack2)
        a.unshift(stack2 + "\n");
      message2 = util.format.apply(util, a);
      var m = {
        id: id++,
        level: lvl,
...
module.exports = ({ options }) => {
  initI18n();
  const log3 = (0, import_logger.getLogger)("server:serve-local");
  const router = import_express.default.Router();
  const configuration = options.configuration;
  const paths = configuration.paths;
  const defaultOptions = Object.fromEntries(Object.entries(configuration).filter(([key]) => {
    return key !== "paths" && key !== "setHeaders";
  }));
  for (const config of paths) {
    const overridenOptions = Object.fromEntries(Object.entries(config).filter(([key]) => {
      return key !== "path" && key !== "src" && key !== "setHeaders";
    }));
    const serveStaticOptions = Object.assign({}, defaultOptions, overridenOptions);
    log3.info(`Serving path ${config.path} locally from ${config.src} with configuration ${JSON.stringify(serveStaticOptions)}`);
